"""Shared fixtures for bot_base tests.

Unit specs use ``MagicMock`` repos + a fake ``IMessengerProvider`` and need no
DB. Integration specs request ``app`` / ``client`` and self-bootstrap a
``<dbname>_test`` database with all core + bot_base tables created via
``db.create_all()`` (mirrors the meinchat plugin's harness).
"""
import os
import sys
...
def _ensure_bot_base_enabled(flask_app) -> None:
    """Enable bot_base (+ peer plugins it needs) so on_enable registrations
...
def app():
    ...
    _ensure_bot_base_enabled(application)
